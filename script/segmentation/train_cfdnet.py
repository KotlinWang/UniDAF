...
from model.cfdnet import Model
...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        with self.fabric.device:
            ...
            self.deep_model = Model(n_channels=9, n_classes=4, bilinear=False)
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in enumerate(loop):
                ...
                clf_output = self.deep_model(input) # If you use SiamCRNN 
                
                final_loss = 0.4 * self.seg_loss(clf_output[0], labels_clf) + 0.5 * self.seg_loss(clf_output[1], labels_clf) +\
                     0.6 * self.seg_loss(clf_output[2], labels_clf) + 0.7 * self.seg_loss(clf_output[3], labels_clf) +\
                        self.seg_loss(clf_output[4], labels_clf)
    ...
    def validation(self):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                output_clf = clf_output[-1].data.cpu().numpy()
...
def main():
    ...
    parser.add_argument('--model_type', type=str, default='segmention/cfdnet')
