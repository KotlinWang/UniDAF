...
from .la import LinearAttention
...
class GFE(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, drop_path=0., norm_layer=nn.LayerNorm):
        super().__init__()
        self.pos_embed = nn.Conv2d(in_dim, in_dim, 3, padding=1, groups=in_dim)
        self.norm1 = norm_layer(in_dim)
        # in_channels, key_channels, head_count, value_channels
        self.attn = MSLA(dim=in_dim, num_heads=num_heads)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(in_dim)
        self.moe = SparseMoE(in_dim, num_experts=6, top_k=2)
        self.fuse_c = BasicConv2d(in_dim, out_dim, 3)
