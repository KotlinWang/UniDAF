...
from .class_ware_whiten import ClassWareWhitening
from .coral import CoralLoss, ImageLevelCoralLoss
from .mmd import MMDLoss, ImageLevelMMDLoss
...
class Aligner:
    ...
    def __init__(self, feat_channels=64, class_num=7, ignore_label=-1, decay=0.999, topk=32, down_scale=16, resume=None):
        ...
        self.downscale_gt = DownscaleLabel(scale_factor=down_scale, n_classes=class_num, ignore_label=ignore_label,
                                           min_ratio=0.75)

        # criterion for domain alignment
        self.coral = CoralLoss()
        self.image_level_coral = ImageLevelCoralLoss()
        self.mmd = MMDLoss()
        self.image_level_mmd = ImageLevelMMDLoss()
    ...
    def align_image(self, feat_s, feat_t, distance='pearson'):
        """ Nonda: image level distribution alignment
        Args:
            feat_s: Tensor, shape=(b, k, h, w)
            feat_t: Tensor, shape=(b, k, h, w), the same as feat_s

        Returns:
            dist_img: distance of the source and target domain.
        """
        assert distance in ['euclidean', 'pearson']
        ...
        feat_s = torch.sum(feat_s, dim=[2, 3])  # (b, k)
        feat_t = torch.sum(feat_t, dim=[2, 3])  # (b, k)
        if distance == 'euclidean':
            dist = torch.cdist(feat_s, feat_t, p=2)
        else:
            dist = self._pearson_dist(feat_s, feat_t, False)
        return torch.trace(dist)

    def align_image_triple(self, ancher, pos, neg, distance='pearson'):
        """ Nonda: image level distribution alignment
        Args:
            feat_s: Tensor, shape=(b, k, h, w)
            feat_t: Tensor, shape=(b, k, h, w), the same as feat_s
        ...
        """
        assert distance in ['euclidean', 'pearson']
        assert ancher.shape == pos.shape, 'tensor "feat_s" has the same shape as tensor "feat_t"'
        assert len(ancher.shape) == 4, 'tensor "feat_s" and "feat_t" must have 4 dimensions'
        bs = ancher.size(0)
        ancher = torch.sum(ancher, dim=[2, 3])  # (b, k)
        pos = torch.sum(pos, dim=[2, 3])  # (b, k)
        neg = torch.sum(neg, dim=[2, 3])  # (b, k)
        if distance == 'euclidean':
            dist_pos = torch.cdist(ancher, pos, p=2)    # (b, b)
            dist_neg = torch.cdist(ancher, neg, p=2)    # (b, b)
        else:
            dist_pos = self._pearson_dist(ancher, pos, False)
            dist_neg = self._pearson_dist(ancher, neg, False)
        return 1.0 + (torch.trace(dist_pos) - torch.trace(dist_neg)) / bs

    def align_domain_coral(self, feat_s, feat_t):
        assert feat_s.shape == feat_t.shape, 'tensor "feat_s" has the same shape as tensor "feat_t"'
        assert len(feat_s.shape) == 4, 'tensor "feat_s" and "feat_t" must have 4 dimensions'
    ...
    def align_domain_coral_imagelevel(self, feat_s, feat_t):
        assert feat_s.shape == feat_t.shape, 'tensor "feat_s" has the same shape as tensor "feat_t"'
        assert len(feat_s.shape) == 4, 'tensor "feat_s" and "feat_t" must have 4 dimensions'
        bs = feat_s.shape[0]
        feat_s = feat_s.permute(0, 2, 3, 1).reshape([bs, -1, self.feat_channels])
        feat_t = feat_t.permute(0, 2, 3, 1).reshape([bs, -1, self.feat_channels])
        return self.image_level_coral(feat_s, feat_t)

    def align_domain_mmd(self, feat_s, feat_t):
        assert feat_s.shape == feat_t.shape, 'tensor "feat_s" has the same shape as tensor "feat_t"'
        assert len(feat_s.shape) == 4, 'tensor "feat_s" and "feat_t" must have 4 dimensions'
        feat_s = tnf.interpolate(feat_s, size=(4, 4), mode='bilinear', align_corners=False)
        feat_t = tnf.interpolate(feat_t, size=(4, 4), mode='bilinear', align_corners=False)
        feat_s = feat_s.permute(0, 2, 3, 1).reshape([-1, self.feat_channels])
        feat_t = feat_t.permute(0, 2, 3, 1).reshape([-1, self.feat_channels])
        return self.mmd(feat_s, feat_t)

    def align_domain_mmd_imagelevel(self, feat_s, feat_t):
        assert feat_s.shape == feat_t.shape, 'tensor "feat_s" has the same shape as tensor "feat_t"'
        assert len(feat_s.shape) == 4, 'tensor "feat_s" and "feat_t" must have 4 dimensions'
        bs = feat_s.shape[0]
        feat_s = tnf.interpolate(feat_s, size=(8, 8), mode='bilinear', align_corners=False)
        feat_t = tnf.interpolate(feat_t, size=(8, 8), mode='bilinear', align_corners=False)
        feat_s = feat_s.permute(0, 2, 3, 1).reshape([bs, -1, self.feat_channels])
        feat_t = feat_t.permute(0, 2, 3, 1).reshape([bs, -1, self.feat_channels])
        return self.image_level_mmd(feat_s, feat_t)

    def update_prototype(self, feat, label):
        """Update global prototypes by source features and labels, and return downscale labels"""
    ...
    def get_prototype_weight_4pixel(self, feats, label_hard, temp=2.0):
        ...
        simi_matrix = 1.0 / self._pearson_dist(_feats, self.prototypes, True)  # (b*h*w, c) Pearson distance
        simi_matrix = simi_matrix.view(b, h, w, -1).permute(0, 3, 1, 2)  # (b, c, h, w)
    ...
    def _class_align_loss(self, prototypes_1, prototypes_2, margin=0.3, hard_ratio=0.3):
        ...
        dist_matrix = self._pearson_dist(prototypes_1, prototypes_2, detach=False)  # (c, c), pearson distances
    ...
    def _instance_align_loss(self, feat, label, margin=0.3, hard_ratio=0.3):
        ...
        dist_matrix = self._pearson_dist(feat, self.prototypes, detach=False)  # Euclidean distance, (b*h*w, c)
    ...
    def _pearson_dist(self, feat1, feat2, detach=True):
        """
        ...
        Args:
            ...
            detach: boolean, if detach the output tensor or not
        Returns:
        ...
        return pearson_dist.detach() if detach else pearson_dist    # (n, m)
