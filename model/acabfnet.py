...
class CrossNet(nn.Module):
    def __init__(self, nclass, head):
        ...
        self.stem_cnn = nn.Sequential(
            nn.Conv2d(9, 64, kernel_size=7, padding=3, stride=2, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(True),
    ...
    #     y = imgs.clone()
        ...
        out2 = self.deconv1(Token2image(stage3_)) + F.interpolate(Token2image(stage3_), scale_factor=2, mode='bilinear',
                                                               align_corners=True)  # 1/8,128
        out3 = self.deconv2(Token2image(stage4_)) + F.interpolate(Token2image(stage4_), scale_factor=4, mode='bilinear',
                                                               align_corners=True)  # 1/8,128
        out = torch.cat([out1, out2, out3], 1)  # 1/8,384
        # =======================================
        out_cnn1 = b2
        out_cnn2 = self.c1(b3)
        out_cnn2 = self.deconv3(out_cnn2) + F.interpolate(out_cnn2, scale_factor=2, mode='bilinear', align_corners=True)
        out_cnn3 = self.c2(b4)
        out_cnn3 = self.deconv4(out_cnn3) + F.interpolate(out_cnn3, scale_factor=4, mode='bilinear', align_corners=True)
        ...
        # ===================================================
        return F.interpolate(OUT, scale_factor=8, mode='bilinear', align_corners=True) + self.deconv5(cat)
