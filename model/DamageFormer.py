...
class DamageFormer(nn.Module):
    def __init__(self, post_in_channels=3):
        super(DamageFormer, self).__init__()

        # if it is multimodal task (if not, please use pure-siamese architecture)
        self.encoder_1 = SwinTransformerFeatureExtractor(swin_transformer = swin_t(weights=Swin_T_Weights.IMAGENET1K_V1), depths=[2, 2, 6, 2]) # getattr(mix_transformer, backbone)()
        self.encoder_2 = SwinTransformerFeatureExtractor(swin_transformer = swin_t(weights=Swin_T_Weights.IMAGENET1K_V1), depths=[2, 2, 6, 2]) # getattr(mix_transformer, backbone)()

        if post_in_channels != 3:
            self.encoder_2.swin_transformer.features[0][0] = nn.Conv2d(post_in_channels, 96, kernel_size=4, stride=4, bias=False)
