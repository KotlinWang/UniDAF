...
class MultimodalDamageAssessmentDatset(Dataset):
    ...
    def load_img_and_mask(self, index):
        if not self.mapping:
            pre_path = os.path.join(self.dataset_path, 'DisasterSet', 'pre-event', self.data_list[index] + '_pre_disaster' + self.suffix)
            post_sar_path = os.path.join(self.dataset_path, 'DisasterSet', 'post-event-sar', self.data_list[index] + '_post_disaster_sar'  + self.suffix)
            post_opt_path = os.path.join(self.dataset_path, 'DisasterSet', 'post-event-opt', self.data_list[index] + '_post_disaster_opt'  + self.suffix)
            label_path = os.path.join(self.dataset_path, 'DisasterSet', 'target', self.data_list[index] + '_building_damage'  + self.suffix)
        else:
