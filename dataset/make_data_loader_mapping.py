...
class MultimodalDamageAssessmentDatset(Dataset):
    def __init__(self, dataset_path, data_list, crop_size, type='train', data_loader=img_loader, suffix='.tif', target_domain=False):
    ...
    def load_img_and_mask(self, index):
        if self.target_domain:
            pre_path = os.path.join(self.dataset_path, 'DisasterSet3', 'pre-event', self.data_list[index] + '_pre_disaster' + self.suffix)
            post_sar_path = os.path.join(self.dataset_path, 'DisasterSet3', 'post-event-sar', self.data_list[index] + '_post_disaster_sar'  + self.suffix)
            post_opt_path = os.path.join(self.dataset_path, 'DisasterSet3', 'post-event-opt', self.data_list[index] + '_post_disaster_opt'  + self.suffix)
            label_path = os.path.join(self.dataset_path, 'DisasterSet3', 'target', self.data_list[index] + '_building_damage'  + self.suffix)
        else:
            pre_path = os.path.join(self.dataset_path, 'LA-WildFire', 'pre-event', self.data_list[index] + '_pre_disaster' + self.suffix)
            post_sar_path = os.path.join(self.dataset_path, 'LA-WildFire', 'post-event-sar', self.data_list[index] + '_post_disaster_sar'  + self.suffix)
            post_opt_path = os.path.join(self.dataset_path, 'LA-WildFire', 'post-event-opt', self.data_list[index] + '_post_disaster_opt'  + self.suffix)
            label_path = os.path.join(self.dataset_path, 'LA-WildFire', 'target', self.data_list[index] + '_building_damage'  + self.suffix)
