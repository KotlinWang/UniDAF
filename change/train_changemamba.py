...
from model.ChangeMamba.STMambaBDA import STMambaBDA
    ...
    def __init__(self, args):
        ...
        with self.fabric.device:
            ...
            self.deep_model = STMambaBDA(
                ...
                pretrained="./model/ChangeMamba/pretrained_weight/vssm_tiny_0230s_ckpt_epoch_264.pth",
                patch_size=4,
                in_chans=3,
                num_classes=1000,
                depths=[ 2, 2, 4, 2 ],  # tiny [ 2, 2, 4, 2 ], base [2, 2, 15, 2]
                dims=96,  # tiny/small 96, base 128
                # ===================
                ssm_d_state=1,
                ssm_ratio=2,
                ssm_rank_ratio=2.0,
                ...
                ssm_drop_rate=0.0,
                ssm_init="v0",
                forward_type="v2",  # tiny v2, base/small v3noz
                # ===================
                mlp_ratio=-0.1,  # tiny -0.1, base/small 4.0
                mlp_act_layer="gelu",
                mlp_drop_rate=0.0,
                # ===================
                drop_path_rate=0.2,  # tiny 0.2, base 0.1
                patch_norm=True,
                norm_layer="ln",
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in enumerate(loop):
                ...
                if not valid_labels_clf:
                    continue
                loc_output, clf_output = self.deep_model(pre_change_imgs, post_opt_change_imgs, post_sar_change_imgs) # If you use SiamCRNN 
                
                final_loss = self.seg_loss(clf_output, labels_clf) + self.seg_loss(loc_output, labels_loc)
    ...
    def validation(self):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                _, clf_output = self.deep_model(pre_change_imgs, post_opt_change_imgs, post_sar_change_imgs)
...
def main():
    ...
    parser.add_argument('--model_type', type=str, default='change/changemamba')
