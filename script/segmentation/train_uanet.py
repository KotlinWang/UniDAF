...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        with self.fabric.device:
            ...
            # pvt_v2_b2_li.in1k
            self.deep_model = UANet_pvt(backbone='pvt_v2_b2_li.in1k', in_channel=9, channel=32, num_classes=4) 
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in enumerate(loop):
                ...
                outputs = self.deep_model(input) # If you use SiamCRNN 
                
                final_loss = self.seg_loss(outputs[0], labels_clf) + self.seg_loss(outputs[1], labels_clf) + \
                    self.seg_loss(outputs[2], labels_clf) + self.seg_loss(outputs[3], labels_clf)
    ...
    def validation(self):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                input = torch.cat([pre_change_imgs, post_opt_change_imgs, post_sar_change_imgs], dim=1)
                outputs = self.deep_model(input)

                output_clf = outputs[-1].data.cpu().numpy()
...
def main():
    ...
    parser.add_argument('--model_type', type=str, default='segmention/uanet')
